...
API_KEY=your_abiscan_api
VRF_ADDRESS=your_chainlink_vrf_address_id
KEY_HASH=your_chainlink_key_hash_id
...
7. Replace `your_private_key` with your private key in the `.env` file.
...
- Replace `your_abiscan_api` with yours in the `.env` file.
...
- Copy “VRF Coordinator” and replace `your_chainlink_vrf_address_id` with yours in the `.env` file. I have pointed out the address in the following image for your ease.
...
- Copy the “Key Hash” address and replace `your_chainlink_key_hash_id` with yours in the `.env` file. I have pointed out the hash in the following image for your ease.
...
- Replace `your_chainlink_vrf_subscription_id` with your ID in the `.env` file.
