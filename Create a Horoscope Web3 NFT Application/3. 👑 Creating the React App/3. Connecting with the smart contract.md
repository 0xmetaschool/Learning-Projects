...
1. Create `abi` folder inside of `src` folder.
2. Create `horoscopeNFT.json` file inside of `abi` folder.
3. Paste the code from `artifacts/contracts/contract-name.json` file present inside of your hardhat project to `horoscopeNFT.json` file.

Here’s how the file structure will look like.
...
Refresh your react app if it is already running in your terminal if not then run `npm start` command and follow the following steps.
